...
from WebCrawling import keyWordsExtraction as kwe
from WebCrawling import get_academic_sources as service
...
def get_results(abstract = "", language = "english" ):
    ...
    for repository in dict_sources :
        print('extracting the bagulho')
        result = repository(searchKeyWords, round(n_pages), language = language)
        extractedReferences += (result.get('references') if result.get('connection') == 'success' else None)
    
    
    
    #service.storeInJson(extractedReferences, "output.json")
